                  ...
                  <div className="p-5 border-t border-white/5 bg-[#0f1015]/50 flex flex-col gap-4 rounded-b-xl">
                    ...
                    <>
                      ...
                      <div className="flex flex-col gap-2 mt-4">
                        <label className="font-label-sm text-[11px] text-white/50 uppercase tracking-wider font-semibold flex items-center justify-between">
                          Fare İmlecini Gizle
                          <button
                            onClick={() => setLocal(s => ({ ...s, hideMouseCursor: !s.hideMouseCursor }))}
                            className={`w-10 h-5 rounded-full relative transition-colors ${local.hideMouseCursor ? 'bg-indigo-500' : 'bg-white/10'}`}
                          >
                            <div className={`absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full transition-transform ${local.hideMouseCursor ? 'translate-x-5' : 'translate-x-0'}`} />
                          </button>
                        </label>
                      </div>
                    </>
